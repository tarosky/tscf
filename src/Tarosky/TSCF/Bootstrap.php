...
class Bootstrap extends Singleton {
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		if ( ! $this->is_editor( $hook_suffix ) ) {
			...
			wp_register_style( 'jquery-ui-mp6', "{$this->url}/lib/jquery-ui-mp6/css/jquery-ui.css", array(), '1.0.2' );
			wp_register_style( 'jquery-ui-timepicker-addon', "{$this->url}/lib/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.css", array( 'jquery-ui-mp6' ), '1.5.5' );
			wp_register_script( 'jquery-ui-timepicker-addon', "{$this->url}/lib/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js", array(
			...
			wp_register_script( 'jquery-ui-timepicker-addon-i18n', $this->url . $i18n_file, array( 'jquery-ui-timepicker-addon' ), '1.5.5', true );
			// Check language and find if exists.
			wp_register_script( 'jquery-live-preview', "{$this->url}/lib/jquery-live-preview/jquery-live-preview.min.js", array( 'jquery' ), '1.1.0', true );
			wp_register_style( 'jquery-live-preview', "{$this->url}/css/livepreview.css", array(), '1.1.0', 'screen' );
			// Select2.
			$has_select2_locale = false;
			wp_register_script( 'select2', "{$this->url}/lib/select2/js/select2.min.js", array( 'jquery' ), '4.0.3', true );
			if ( 0 !== strpos( get_locale(), 'en' ) ) {
				...
				if ( file_exists( "{$this->root_dir}/assets/lib/select2/i18n/{$lang}.js" ) ) {
					...
					wp_register_script( 'select2-local', "{$this->url}/lib/select2/js/i18n/{$lang}.js", array( 'select2' ), '4.0.3', true );
				}
			}
			wp_register_style( 'select2', "{$this->url}/lib/select2/css/select2.min.css", array(), '4.0.3' );
			// AceEditor.
			wp_register_script( 'ace-editor', "{$this->url}/lib/ace/ace.js", array(), '1.2.6', true );
			// Every page.
			wp_enqueue_style( 'tscf-admin', $this->url . '/css/tscf-admin.css', array(
				'jquery-ui-timepicker-addon',
				'jquery-live-preview',
				'select2',
			), tscf_version() );
			wp_enqueue_script( 'tscf-helper', $this->url . '/js/dist/tscf-helper.js', array(
			...
			wp_localize_script( 'tscf-helper', 'TSCF', array(
