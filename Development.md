...
前提:
- Node.js 18+ / PHP 7.4+ / Composer / Docker Desktop（@wordpress/env）
...
TSCF は「設定ファイル駆動」のカスタムフィールド管理を採用:

1. JSON によるフィールド定義の一元管理
2. Git によるバージョン管理
3. 子テーマでの設定オーバーライド
4. 大規模案件でもスケーラブル
...
コミット前に、開発・検証用の一時ファイルを削除または .gitignore へ:
- 削除対象例: `test-*.{txt,js}`, `*-test.{js,txt}`, `*.test.{js,txt}`, `debug-*`, `tmp-*`, `sample-*`
- 例外: 正式なテスト（tests/ 配下）、ドキュメント用サンプル、プロダクションで必要なファイル

チェック手順:
1) git status で追加ファイルを確認
2) 該当があれば削除または .gitignore 追加
3) 必要に応じ .gitignore 更新
